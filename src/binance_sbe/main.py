...
from binance_sbe.sbe_decoder import decode
...
def _configure_logging(cfg: AppConfig) -> None:
    ...
    structlog.configure(
        ...
        wrapper_class=structlog.make_filtering_bound_logger(
            structlog.stdlib._NAME_TO_LEVEL.get(
...
class FeedHandlerApp:
    """Top‑level orchestrator for the Binance SBE feed handler."""
    ...
    def __init__(self, config: AppConfig) -> None:
        ...
        self._connector = BinanceConnector(
            ...
            on_binary_frame=self._on_binary_frame,
        )
        self._connector_task: asyncio.Task | None = None

    # ------------------------------------------------------------------
    # Binary frame callback (hot path)
    # ------------------------------------------------------------------

    async def _on_binary_frame(self, frame: bytes) -> None:
        """Called for every binary WS frame from Binance."""
        event = decode(frame)
        if event is None:
            return  # unknown / error — already logged by decoder
    ...
    async def run(self) -> None:
        ...
        # Wait until the connector finishes (cancelled or max retries)
        try:
